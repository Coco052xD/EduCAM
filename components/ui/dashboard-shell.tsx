...
export function DashboardShell({ displayName, children }: { displayName: string; children: React.ReactNode }) {
  return <div className="min-h-screen lg:grid lg:grid-cols-[250px_1fr]">
    ...
    <div>
      <header className="no-print sticky top-0 z-20 flex items-center justify-between border-b border-[#dce4df] bg-white/95 px-4 py-3 backdrop-blur lg:hidden"><Link href="/dashboard" className="font-extrabold">Aula Puente IA</Link><nav className="flex gap-2"><Link aria-label="Grupos" className="rounded-lg p-2" href="/dashboard/groups"><UsersRound/></Link><Link aria-label="Nueva recomendación" className="rounded-lg bg-[#176b5c] p-2 text-white" href="/dashboard/recommendations/new"><BookOpen/></Link></nav></header>
