...
export default function Home() {
  return <main className="landing-page">
    <header className="container-app flex items-center justify-between py-6">
      <Link href="/" className="flex items-center gap-3 font-extrabold"><span className="brand-mark">AP</span>Aula Puente IA</Link>
    ...
    <section className="container-app grid min-h-[72vh] items-center gap-12 py-16 lg:grid-cols-[1.1fr_.9fr]">
      <div>
        ...
        <h1 className="text-5xl font-black leading-[.98] tracking-[-.055em] sm:text-7xl">Cada actividad puede <span className="hero-title-accent">encontrar otra forma</span> de llegar.</h1>
      ...
      <div className="card hero-card relative overflow-hidden p-7 sm:p-10">
        <div className="absolute -right-12 -top-12 size-44 rounded-full border-[2rem] border-[#fde047]/70"/>
        <div className="absolute -bottom-8 -left-8 size-24 rotate-12 rounded-3xl bg-[#ef4444]/15"/>
        <p className="eyebrow">El ciclo de mejora</p>
        <div className="mt-6 grid gap-4">
          {[ [BookOpenCheck,"Partir del contexto","Perfil de aprendizaje y objetivo curricular."], [Sparkles,"Explorar opciones","Propuestas visuales, manipulativas y colaborativas."], [ShieldCheck,"Decidir y mejorar","Tú revisas, aplicas y compartes qué funcionó."] ].map(([Icon,title,text]) => <div key={String(title)} className="learning-step flex gap-4"><Icon className="mt-1 shrink-0"/><div><h2 className="font-extrabold">{title as string}</h2><p className="muted mt-1 text-sm leading-6">{text as string}</p></div></div>)}
        </div>
