...
export default async function GroupPage({ params }: { params: Promise<{ groupId: string }> }) {
  ...
  return <div className="grid gap-9"><PageHeader eyebrow={`${group.grade}.º de primaria`} title={group.name} description={`Ciclo ${group.school_year}`} action={<div className="flex flex-wrap gap-2"><Link className="button button-secondary" href={`/dashboard/students/new?groupId=${group.id}`}><Plus size={18}/>Agregar alumno</Link><Link className="button" href="/dashboard/recommendations/new"><Sparkles size={18}/>Nueva recomendación</Link></div>}/>
    <section><h2 className="section-title mb-4">Alumnos</h2>{students.length ? <div className="grid-cards">{students.map((student) => { const complete = (student.student_profile_answers?.[0]?.count ?? 0) > 0; return <Link href={`/dashboard/students/${student.id}`} className="card card-link p-5 transition hover:-translate-y-1" key={student.id}><div className="flex items-center justify-between gap-2"><h3 className="font-extrabold">{student.name}</h3><span className={`chip ${complete ? "" : "!border-[#fca5a5] !bg-[#fef2f2] !text-[#991b1b]"}`}>{complete ? "Perfil completo" : "Perfil pendiente"}</span></div><p className="muted mt-3 text-sm">{student.age_range} años · {student.grade}.º</p></Link>; })}</div> : <EmptyState title="No hay alumnos en este grupo" description="Agrega un alumno con los datos mínimos para planear." action={<Link className="button" href={`/dashboard/students/new?groupId=${group.id}`}>Agregar alumno</Link>}/>}</section>
