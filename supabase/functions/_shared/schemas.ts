...
export const requestSchema = z.object({ requestId: z.uuid(), refinement: z.string().trim().max(300).optional() });
export const generatedActivitySchema = z.object({
  title: z.string().min(5).max(150),
  activityType: z.enum(["visual", "manipulative", "collaborative", "playful", "mixed"]),
  objective: z.string().min(10).max(500), durationMinutes: z.number().int().min(5).max(180),
  materials: z.array(z.string()).max(20), preparation: z.array(z.string()).max(10),
  steps: z.array(z.object({ order: z.number().int(), instruction: z.string(), estimatedMinutes: z.number().int().min(1), educatorSupport: z.string() })).min(2).max(12),
...
export const responseSchema = z.object({ proposals: z.array(generatedActivitySchema).min(1).max(3) });
